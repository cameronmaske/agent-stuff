...
import { getCandidateUrls } from "./browser-utils.js";
...
function connectWebSocket(wsUrl, timeout = 5000) {
...
export async function connect(timeout = 5000) {
  const candidates = getCandidateUrls().map(normalizeBaseUrl).filter(Boolean);
  let lastError;

  for (const baseUrl of candidates) {
    try {
      const info = await fetchJson(`${baseUrl}/json/version`, timeout);
  ...
  const error = new Error(
    `Connection timeout - is Chrome running with --remote-debugging-port=9222? Tried: ${tried}`
  );
