 ...
 * - `/review stack` - review current branch in a stack (uses `gt parent`)
 *
...
type ReviewTarget =
	...
	| { type: "stack"; parentBranch: string; currentBranch: string };
...
1. If any \`apps/*/*.py\` files are modified, run the tests for that application module to ensure they pass:
   \`docker compose run --rm -q app pytest apps/<module>/\`
   (where \`<module>\` is the app directory name, e.g., \`apps/users/\` → \`pytest apps/users/\`)
2. If any new test files are added (files matching \`test_*.py\` or \`*_test.py\`), verify they run and pass.
3. Flag as [P0] if tests fail - code changes that break tests should not be merged.

## Project-specific rules

Before reviewing, check if \`.claude/rules/\` exists and scan for applicable rules:
1. List files in \`.claude/rules/\` (including subdirectories like \`.claude/rules/backend/\`, \`.claude/rules/frontend/\`)
2. Based on the files changed in the diff, read any relevant rule files (e.g., if \`services.py\` is modified, check \`.claude/rules/backend/services.md\`)
3. Apply any constraints, patterns, or requirements from those rules when reviewing the changes
4. Flag violations of project-specific rules with appropriate priority levels
...
/**
 * Get the parent branch in a stack using Graphite CLI (gt parent)
 */
async function getStackParent(pi: ExtensionAPI): Promise<string | null> {
	const { stdout, code } = await pi.exec("gt", ["parent"]);
	if (code === 0 && stdout.trim()) {
		return stdout.trim();
	}
	return null;
 ...
 */
async function hasGraphiteCli(pi: ExtensionAPI): Promise<boolean> {
	const { code } = await pi.exec("gt", ["--version"]);
	return code === 0;
}
...
async function buildReviewPrompt(pi: ExtensionAPI, target: ReviewTarget): Promise<string> {
	switch (target.type) {
		...
		case "stack":
			return STACK_PROMPT
				.replace(/{currentBranch}/g, target.currentBranch)
				.replace(/{parentBranch}/g, target.parentBranch);
	}
}
...
function getUserFacingHint(target: ReviewTarget): string {
	switch (target.type) {
		...
		case "stack":
			return `stack: '${target.currentBranch}' on '${target.parentBranch}'`;
	}
}
...
const REVIEW_PRESETS = [
	...
	{ value: "stack", label: "Review current stacked branch", description: "(Graphite gt)" },
...
export default function reviewExtension(pi: ExtensionAPI) {
	 ...
	 */
	async function getSmartDefault(): Promise<"uncommitted" | "baseBranch" | "stack" | "commit"> {
		// Priority 1: If there are uncommitted changes, default to reviewing them
		if (await hasUncommittedChanges(pi)) {
			return "uncommitted";
		}

		// Priority 2: If on a stacked branch (Graphite), default to stack review
		if (await hasGraphiteCli(pi)) {
			const parentBranch = await getStackParent(pi);
			if (parentBranch) {
				return "stack";
			}
		}

		// Priority 3: If on a feature branch (not the default branch), default to PR-style review
		...
		// Priority 4: Default to reviewing a specific commit
		return "commit";
	}
	...
	async function showReviewSelector(ctx: ExtensionContext): Promise<ReviewTarget | null> {
		...
		while (true) {
			...
			switch (result) {
				...
				case "stack": {
					const target = await getStackTarget(ctx);
					if (target) return target;
					break;
				}
	...
	/**
	 * Get stack target (current branch vs parent in stack)
	 */
	async function getStackTarget(ctx: ExtensionContext): Promise<ReviewTarget | null> {
		// Check if Graphite CLI is available
		if (!(await hasGraphiteCli(pi))) {
			ctx.ui.notify("Graphite CLI (gt) not found. Install it from https://graphite.dev", "error");
			return null;
		}
		...
		// Get parent branch from Graphite
		const parentBranch = await getStackParent(pi);
		if (!parentBranch) {
			ctx.ui.notify("No parent branch found. Are you on a stacked branch?", "error");
			return null;
		}
	 ...
	 * Returns the target or a special marker for PR/stack that needs async handling
	 */
	function parseArgs(args: string | undefined): ReviewTarget | { type: "pr"; ref: string } | { type: "stackMarker" } | null {
	...
	pi.registerCommand("review", {
		...
		handler: async (args, ctx) => {
			...
			if (parsed) {
				...
				} else if (parsed.type === "stackMarker") {
					// Handle stack review (async operation)
					target = await getStackTarget(ctx);
					if (!target) {
						ctx.ui.notify("Stack review failed. Returning to review menu.", "warning");
					}
				} else {
					target = parsed;
				}
