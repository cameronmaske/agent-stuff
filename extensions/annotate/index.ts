...
function resolveConfigPath(): string {
  ...
  if (isWsl()) {
    ...
    const winUser = process.env.WIN_USERNAME || process.env.USERNAME || process.env.USER;
    if (winUser) {
      candidates.push(`/mnt/c/Users/${winUser}/pi-annotate.json`);
    }
    candidates.push("/mnt/c/Users/Public/pi-annotate.json");
    for (const candidate of candidates) {
      if (fs.existsSync(candidate)) return candidate;
    }
    const discovered = findConfigInWindowsUsers();
    if (discovered) return discovered;
    return candidates[0] || "/mnt/c/Users/Public/pi-annotate.json";
  }
  return path.join(os.tmpdir(), "pi-annotate.json");
}

function findConfigInWindowsUsers(): string | null {
  try {
    const entries = fs.readdirSync("/mnt/c/Users", { withFileTypes: true });
    for (const entry of entries) {
      if (!entry.isDirectory()) continue;
      const candidate = `/mnt/c/Users/${entry.name}/pi-annotate.json`;
      if (fs.existsSync(candidate)) return candidate;
    }
  } catch {
    return null;
...
function loadTcpConfig(): TcpConfig | null {
  try {
    const raw = fs.readFileSync(CONFIG_PATH, "utf8");
    const parsed = JSON.parse(raw) as Partial<TcpConfig>;
    if (parsed.transport !== "tcp") return null;
    if (typeof parsed.token !== "string" || typeof parsed.port !== "number") return null;
    return { transport: "tcp", token: parsed.token, port: parsed.port };
  } catch {
    return null;
  }
}

function getWindowsHostFromResolvConf(): string | null {
  try {
...
function getWindowsHostFromRoute(): string | null {
  try {
...
function getDefaultHost(): string {
  if (process.env.PI_ANNOTATE_HOST) return process.env.PI_ANNOTATE_HOST;
  if (isWsl()) {
    const gateway = getWindowsHostFromRoute();
    if (gateway) return gateway;
    const nameserver = getWindowsHostFromResolvConf();
    if (nameserver) return nameserver;
  }
  return "127.0.0.1";
}
...
export default function (pi: ExtensionAPI) {
  ...
  function setStatus(message: string) {
    ...
    if (!normalized) {
      currentCtx.ui.setStatus("pi-annotate", undefined);
      return;
    }

    const text = currentCtx.ui?.theme?.fg
      ? currentCtx.ui.theme.fg("dim", normalized)
      : normalized;

    currentCtx.ui.setStatus("pi-annotate", text);
  }
  ...
  function connectToHost(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      const transportOverride = process.env.PI_ANNOTATE_TRANSPORT?.toLowerCase();
      let transport: "tcp" | "unix" = "unix";
      let tcpConfig: TcpConfig | null = null;

      if (transportOverride === "tcp") {
        tcpConfig = loadTcpConfig();
        if (!tcpConfig) {
          reject(new Error(`Missing TCP config at ${CONFIG_PATH}. Start the Windows native host or set PI_ANNOTATE_CONFIG_PATH.`));
          return;
        }
        transport = "tcp";
      } else if (transportOverride === "unix") {
        transport = "unix";
      } else {
        tcpConfig = loadTcpConfig();
        if (tcpConfig) {
      ...
      if (transport === "tcp") {
        if (!tcpConfig) {
          reject(new Error("TCP transport selected but no config found."));
          return;
        }
        authToken = tcpConfig.token;
        const host = getDefaultHost();
        browserSocket = net.createConnection({ host, port: tcpConfig.port });
      } else {
        try {
