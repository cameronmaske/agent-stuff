...
const showActionSelector = async (
...
): Promise<"reveal" | "quicklook" | "open" | "edit" | "addToPrompt" | "diffDelta" | "diffDifftastic" | null> => {
	const actions: SelectItem[] = [
		...(options.canDiff
			? [
					{ value: "diffDelta", label: "Diff in delta" },
					{ value: "diffDifftastic", label: "Diff in difftastic" },
				]
			: []),
	...
	return ctx.ui.custom<
		"reveal" | "quicklook" | "open" | "edit" | "addToPrompt" | "diffDelta" | "diffDifftastic" | null
	>((tui, theme, _kb, done) => {
		...
		selectList.onSelect = (item) =>
			done(item.value as "reveal" | "quicklook" | "open" | "edit" | "addToPrompt" | "diffDelta" | "diffDifftastic");
...
const openDiff = async (ctx: ExtensionContext, target: FileEntry, gitRoot: string | null): Promise<void> => {
	if (!gitRoot) {
	...
	const command = "git diff --color=always HEAD -- \"$PI_DIFF_PATH\" | delta --paging=always --side-by-side";
	...
	await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
		...
		const result = spawnSync("bash", ["-lc", command], {
			stdio: "inherit",
			cwd: gitRoot,
			env: { ...process.env, PI_DIFF_PATH: relativePath },
		});
	...
	if (errorMessage) {
		ctx.ui.notify(`Failed to run delta for ${target.displayPath}: ${errorMessage}`, "error");
		return;
	}

	if (exitCode !== null && exitCode !== 0) {
		ctx.ui.notify(`delta exited with code ${exitCode}`, "warning");
	}
};

const openDifftastic = async (ctx: ExtensionContext, target: FileEntry, gitRoot: string | null): Promise<void> => {
	if (!gitRoot) {
		ctx.ui.notify("Git repository not found", "warning");
		return;
	}
	...
	const relativePath = path.relative(gitRoot, target.resolvedPath).split(path.sep).join("/");
	const command = "GIT_EXTERNAL_DIFF='difft --display side-by-side' git diff --ext-diff HEAD -- \"$PI_DIFF_PATH\"";
	...
	if (errorMessage) {
		ctx.ui.notify(`Failed to run difftastic for ${target.displayPath}: ${errorMessage}`, "error");
		return;
	}

	if (exitCode !== null && exitCode !== 0) {
		ctx.ui.notify(`difftastic exited with code ${exitCode}`, "warning");
	}
};
...
const showFileSelector = async (
	...
	gitRoot?: string | null,
): Promise<{ selected: FileEntry | null; quickAction: "diffDelta" | null }> => {
	...
	let quickAction: "diffDelta" | null = null;
	const selection = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
		...
		container.addChild(
			new Text(theme.fg("dim", "Type to filter • enter to select • ctrl+shift+d delta diff • esc to cancel"), 0, 0),
		);
		container.addChild(new DynamicBorder((str) => theme.fg("accent", str)));
...
const runFileBrowser = async (pi: ExtensionAPI, ctx: ExtensionContext): Promise<void> => {
	...
	while (true) {
		...
		if (quickAction === "diffDelta") {
			await openDiff(ctx, selected, gitRoot);
			continue;
		}
		...
		switch (action) {
			...
			case "diffDelta":
				...
				break;
			case "diffDifftastic":
				await openDifftastic(ctx, selected, gitRoot);
				break;
			default:
