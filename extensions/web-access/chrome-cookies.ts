...
const MAC_CHROME_COOKIES_PATH = join(
	homedir(),
	"Library/Application Support/Google/Chrome/Default/Cookies",
);

const LINUX_CHROME_COOKIE_PATHS = [
	join(homedir(), ".config/google-chrome-wsl/Default/Cookies"),
	join(homedir(), ".config/google-chrome/Default/Cookies"),
	join(homedir(), ".config/google-chrome-beta/Default/Cookies"),
	join(homedir(), ".config/google-chrome-unstable/Default/Cookies"),
...
export async function getGoogleCookies(): Promise<{ cookies: CookieMap; warnings: string[] } | null> {
	...
	if (currentPlatform === "darwin") return getMacCookies();
	if (currentPlatform === "linux") return getLinuxCookies();
	return null;
}

async function getMacCookies(): Promise<{ cookies: CookieMap; warnings: string[] } | null> {
	if (!existsSync(MAC_CHROME_COOKIES_PATH)) return null;
	...
	return readCookiesFromDb(MAC_CHROME_COOKIES_PATH, key, warnings);
}

async function getLinuxCookies(): Promise<{ cookies: CookieMap; warnings: string[] } | null> {
	const cookiesPath = findLinuxCookiesPath();
	if (!cookiesPath) return null;

	const { password, warnings } = await readLinuxSafeStoragePassword();
	const key = pbkdf2Sync(password, "saltysalt", 1, 16, "sha1");
	return readCookiesFromDb(cookiesPath, key, warnings);
}

function findLinuxCookiesPath(): string | null {
	for (const candidate of LINUX_CHROME_COOKIE_PATHS) {
		if (existsSync(candidate)) return candidate;
	}
	return null;
}

async function readLinuxSafeStoragePassword(): Promise<{ password: string; warnings: string[] }> {
	const warnings: string[] = [];
	const candidates = ["chrome", "chromium", "google-chrome"];

	for (const app of candidates) {
		const secret = await readSecretToolPassword(app);
		if (secret) return { password: secret, warnings };
	}

	warnings.push(
		"Could not read Chrome Safe Storage password via secret-tool; falling back to 'peanuts'. " +
			"Install libsecret-tools or run Chrome with --password-store=basic if decryption fails.",
	);
	return { password: "peanuts", warnings };
}

function readSecretToolPassword(appName: string): Promise<string | null> {
