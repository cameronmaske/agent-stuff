...
An extension for [Pi coding agent](https://github.com/badlogic/pi-mono/) that gives Pi web capabilities: search via Perplexity AI, Gemini, Brave Search, or hybrid mode (Gemini + Brave), fetch and extract content from URLs, clone GitHub repos for local exploration, read PDFs, understand YouTube videos, and analyze local video files.
...
You can configure any combination. In `auto` mode (default), the extension tries Perplexity first (if configured), then Gemini API, then Gemini Web via Chrome cookies, then Brave (if configured).
...
Search the web via Perplexity AI, Gemini, Brave Search, or hybrid mode (Gemini + Brave in parallel). Returns synthesized answer with source citations.
...
// Explicit provider
web_search({ query: "...", provider: "gemini" })  // auto, perplexity, gemini, brave, hybrid
...
Provider selection in `auto` mode: Perplexity (if key configured) → Gemini API (if key configured, uses Google Search grounding) → Gemini Web (if signed into Chrome) → Brave (if key configured). Gemini API returns structured citations with source mappings. Gemini Web returns markdown with embedded links. Hybrid mode runs Gemini + Brave in parallel, keeps Gemini as primary, and appends Brave top-3 supplemental sources.
...
All fields are optional. `GEMINI_API_KEY`, `PERPLEXITY_API_KEY`, and `BRAVE_API_KEY` env vars take precedence over config file values. Set `"enabled": false` under `githubClone`, `youtube`, or `video` to disable those features.

`searchProvider` controls `web_search` default: `"auto"` (Perplexity → Gemini API → Gemini Web → Brave), `"perplexity"`, `"gemini"` (API → Web), `"brave"`, or `"hybrid"` (Gemini + Brave parallel).

## Rate Limits

- **Perplexity API**: 10 requests/minute (enforced client-side)
- **Brave Search API**: server-side quota/rate limits (hybrid mode surfaces errors but still returns Gemini results)
...
| `brave.ts` | Brave Search API client and query/domain/recency mapping |
| `gemini-search.ts` | Search routing and provider orchestration (Gemini, Perplexity, Brave, hybrid) |
