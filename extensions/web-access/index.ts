...
function formatSearchSummary(queryData: QueryResultData): string {
	...
	if (queryData.answer) {
		sections.push(queryData.answer);
	}

	const providerResults = queryData.providerResults ?? [];
	const primaryProvider = queryData.primaryProvider;
	const primaryResult = providerResults.find((r) => r.provider === primaryProvider) ?? providerResults[0];
	const primarySources = primaryResult?.results ?? queryData.results;

	if (primarySources.length > 0) {
		sections.push(`**Sources:**\n${formatResultList(primarySources)}`);
	} else {
		sections.push("**Sources:**\n_No sources found._");
	}

	for (const result of providerResults) {
		if (primaryResult && result.provider === primaryResult.provider) continue;
		if (result.results.length > 0) {
			sections.push(`**${providerLabel(result.provider)} (top ${result.results.length}):**\n${formatResultList(result.results)}`);
		} else if (result.error) {
			sections.push(`**${providerLabel(result.provider)}:**\n- Error: ${result.error}`);
		}
	}

	if (queryData.warnings?.length) {
		sections.push(`**Warnings:**\n${queryData.warnings.map((w) => `- ${w}`).join("\n")}`);
	}
...
function formatFullResults(queryData: QueryResultData): string {
	...
	if (providerResults.length > 0) {
		const primaryResult = providerResults.find((r) => r.provider === queryData.primaryProvider) ?? providerResults[0];
		if (primaryResult?.results.length) {
			output += `### ${providerLabel(primaryResult.provider)} sources\n`;
			for (const r of primaryResult.results) {
		...
		for (const result of providerResults) {
			if (primaryResult && result.provider === primaryResult.provider) continue;
			if (result.results.length > 0) {
				output += `### ${providerLabel(result.provider)} (top ${result.results.length})\n`;
				for (const r of result.results) {
					output += `- ${r.title || r.url}\n  ${r.url}\n`;
				}
				output += "\n";
			} else if (result.error) {
				output += `### ${providerLabel(result.provider)}\n- Error: ${result.error}\n\n`;
			}
		}
	} else {
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		label: "Web Search",
		description:
			"Search the web using Perplexity AI, Gemini, Brave Search, or hybrid mode (Gemini + Brave in parallel). Returns an AI-synthesized answer with source citations, plus optional Brave top-3 supplemental results in hybrid mode. Supports batch searching with multiple queries. When includeContent is true, full page content is fetched in the background. Provider auto-selects: Perplexity if configured, else Gemini API (needs key), else Gemini Web (needs Chrome login), else Brave (needs API key).",
		parameters: Type.Object({
			...
			provider: Type.Optional(
				StringEnum(["auto", "perplexity", "gemini", "brave", "hybrid"], {
		...
		async execute(_toolCallId, params, signal, onUpdate, _ctx) {
			...
			for (let i = 0; i < queryList.length; i++) {
				...
				try {
					...
					totalPrimaryResults += response.primaryResultCount ?? response.results.length;
					searchResults.push({
						query,
						...
						primaryResultCount: response.primaryResultCount,
						providerResults: response.providerResults?.map((r) => ({
							provider: r.provider,
							answer: r.answer,
							results: r.results,
							error: r.error ?? null,
			...
			const totalResults = totalPrimaryResults;
			...
			for (const result of searchResults) {
				...
				const hasProviderResults = result.providerResults?.some((r) => r.results.length > 0) ?? false;
				const hasResults = result.results.length > 0 || hasProviderResults;
				if (!hasResults && !result.answer.trim()) {
					output += "No results found.\n\n";
				} else {
					output += formatSearchSummary(result) + "\n\n";
				}
			}
		...
		async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
			...
			if (data.type === "search" && data.queries) {
				...
				return {
					...
					details: { query: queryData.query, resultCount: queryData.primaryResultCount ?? queryData.results.length },
				};
			}
			...
			if (action === "Delete") {
				...
				if (selected.type === "search" && selected.queries) {
					...
					for (const q of queries) {
						const count = q.primaryResultCount ?? q.results.length;
						info += `- "${q.query}" (${count} results)\n`;
					}
